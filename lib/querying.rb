...
def select_series_title_with_most_human_characters
  "SELECT series.title, COUNT(*) FROM series JOIN characters ON characters.series_id = series.id WHERE characters.species = 'human' GROUP BY series.title ORDER BY COUNT(*) DESC LIMIT 1 "
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, character_books.character_id FROM characters JOIN character_books ON character_books.character_id = characters.id ORDER BY "
end
